...
local function newClient()
    ...
    function client:pullEnd(id)
        self:fire("ENCOUNTER_END", id, "Boss", 16, 20, 1)
    end
    function client:wipe(id)
        self:fire("ENCOUNTER_END", id, "Boss", 16, 20, 0)
    end
    function client:zone(instanceType)
        self.instanceType = instanceType
        self:fire("PLAYER_ENTERING_WORLD", false, false)
    end
...
test("re-pulling after a wipe writes nothing, and the kill restores the originals", function()
    ...
    c:pullStart(3132)
    c:wipe(3132)
    local writes = c.writes
    c:pullStart(3132)
    eq(c.writes, writes, "writes at the re-pull")
    c:pullEnd(3132)
    eq(c.cvars.raidGraphicsParticleDensity, "3", "particle density after the kill")
    eq(c.cvars.ffxDeath, "1", "ffxDeath after the kill")
    eq(EncounterSettingsDB.active, nil, "active overrides after the kill")
end)

test("pulling a different boss after a wipe restores what it does not need", function()
    local c = configuredClient()
    c:slash("set 3133 ffxDeath 0")
    c:pullStart(3132)
    c:wipe(3132)
    c:pullStart(3133)
    eq(c.cvars.raidGraphicsParticleDensity, "3", "particle density during the other boss")
    eq(c.cvars.ffxDeath, "0", "ffxDeath during the other boss")
    c:pullEnd(3133)
    eq(c.cvars.ffxDeath, "1", "ffxDeath after the other boss")
    c:pullStart(3132)
    c:wipe(3132)
    c:pullStart(1)
    eq(c.cvars.raidGraphicsParticleDensity, "3", "particle density during an unconfigured boss")
    eq(c.cvars.ffxDeath, "1", "ffxDeath during an unconfigured boss")
end)

test("a configuration change between wipe and re-pull is applied and the original survives", function()
    local c = configuredClient()
    c:pullStart(3132)
    c:wipe(3132)
    c:slash("set 3132 graphicsParticleDensity 1")
    c:pullStart(3132)
    eq(c.cvars.raidGraphicsParticleDensity, "1", "particle density at the re-pull")
    c:pullEnd(3132)
    eq(c.cvars.raidGraphicsParticleDensity, "3", "particle density after the kill")
end)

test("a value the user changes between wipe and re-pull becomes the new original", function()
    local c = configuredClient()
    c:pullStart(3132)
    c:wipe(3132)
    c.cvars.raidGraphicsParticleDensity = "5"
    c:pullStart(3132)
    eq(c.cvars.raidGraphicsParticleDensity, "0", "particle density at the re-pull")
    c:pullEnd(3132)
    eq(c.cvars.raidGraphicsParticleDensity, "5", "particle density after the kill")
end)

test("leaving the instance after a wipe restores the settings", function()
    local c = configuredClient()
    c:pullStart(3132)
    c:wipe(3132)
    c:zone("none")
    eq(c.cvars.raidGraphicsParticleDensity, "3", "particle density after leaving")
    eq(c.cvars.ffxDeath, "1", "ffxDeath after leaving")
    eq(EncounterSettingsDB.active, nil, "active overrides after leaving")
end)

test("logging back in after a wipe restores the settings", function()
    local c = configuredClient()
    c:pullStart(3132)
    c:wipe(3132)
    c:login()
